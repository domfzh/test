...
class DirectoryIterator_DIV2K(Iterator):
    def __init__(self,
        ...
        import glob
        lrs = glob.glob(datadir+'/train_LR/X{}/*.PNG'.format(scale_factor))
        lrs.sort()
    ...
    def next(self):
        ...
        if self.shuffle and self.first_run:
            ...
            if self.shuffle:
                self.lr_jpgs, self.sharp_jpgs = self.shuffle_list(self.lr_jpgs, self.sharp_jpgs)
        ...
        while (len(batch_blur) < self.out_batch_size):
            blurs = self.lr_jpgs[(current_index+i) % self.total_count]
            sharps = self.sharp_jpgs[(current_index+i) % self.total_count]

            # check if jpg file is damaged
            try:
