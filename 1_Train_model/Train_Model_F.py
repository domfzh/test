...
I_SAVE = 100       # save models every N iteration
...
for i in tqdm(range(START_ITER+1, NB_ITER+1)):
    ...
    batch_F1 = model_G(F.pad(batch_L, (0,1,0,1), mode='reflect'))

    batch_F2 = model_G(F.pad(torch.rot90(batch_L, 1, [2,3]), (0,1,0,1), mode='reflect'))
    batch_F2 = torch.rot90(batch_F2, 3, [2,3])

    batch_F3 = model_G(F.pad(torch.rot90(batch_L, 2, [2,3]), (0,1,0,1), mode='reflect'))
    batch_F3 = torch.rot90(batch_F3, 2, [2,3])

    batch_S4 = model_G(F.pad(torch.rot90(batch_L, 3, [2,3]), (0,1,0,1), mode='reflect'))
    batch_S4 = torch.rot90(batch_S4, 1, [2,3])


    batch_F = ( torch.clamp(batch_F1,-1,1)*127 + torch.clamp(batch_F2,-1,1)*127 )
    batch_F += ( torch.clamp(batch_F3,-1,1)*127 + torch.clamp(batch_S4,-1,1)*127 )
    batch_F /= 255.0

    loss_Pixel = torch.mean( ((batch_F - batch_H)**2)  )
    loss_G = loss_Pixel
    ...
    if i % I_VALIDATION == 0:
        with torch.no_grad():
            ...
            files_gt = glob.glob(VAL_DIR + '/HR_x3/*.png')
            files_gt.sort()
            files_lr = glob.glob(VAL_DIR + '/LR_x3/*.png')
            files_lr.sort()
            ...
            for ti, fn in enumerate(files_gt):
                ...
                print(val_L.shape)
                # Run model
                batch_F1 = model_G(F.pad(val_L, (0,1,0,1), mode='reflect'))
                print(batch_F1.shape)

                batch_F2 = model_G(F.pad(torch.rot90(val_L, 1, [2,3]), (0,1,0,1), mode='reflect'))
                batch_F2 = torch.rot90(batch_F2, 3, [2,3])

                batch_F3 = model_G(F.pad(torch.rot90(val_L, 2, [2,3]), (0,1,0,1), mode='reflect'))
                batch_F3 = torch.rot90(batch_F3, 2, [2,3])
                ...
                batch_F = ( torch.clamp(batch_F1,-1,1)*127 + torch.clamp(batch_F2,-1,1)*127 )
                batch_F += torch.clamp(batch_F3,-1,1)*127
                batch_F /= 255.0


                # Output 
                image_out = (batch_F).cpu().data.numpy()
                ...
                CROP_F = 3
                psnrs.append(PSNR(_rgb2ycbcr(img_gt)[:,:,0], _rgb2ycbcr(image_out)[:,:,0], CROP_F))
